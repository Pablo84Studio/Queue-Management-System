                ...
                <div class="counter-box ${statusClass}">
                    ...
                    <div style="margin-top: 10px;">
                        ${counter.status === 'closed' ? 
                            `<button class="btn btn-sm btn-success" onclick="openCounter(${counter.id})">Open</button>` : 
                            `<button class="btn btn-sm btn-warning" onclick="closeCounter(${counter.id})">Close</button>`
                        }
...
async function loadServicesTable() {
    ...
    try {
        ...
        try {
            ...
            services = [
                { id: 1, name: 'General Inquiry', status: 'active' },
                { id: 2, name: 'Registration', status: 'active' },
                { id: 3, name: 'Payment Processing', status: 'active' },
                { id: 4, name: 'Troubleshooting', status: 'active' },
                { id: 5, name: 'Document Submission', status: 'active' }
            ];
        }

        let html = '';
        for (const service of services) {
            // Count counters for this service
            const countersResponse = await api(`../api/counters.php?action=get_by_service&service_id=${service.id}`, 'GET');
            const counterCount = countersResponse.success ? (countersResponse.data ? countersResponse.data.length : 0) : 0;

            html += `
                <tr>
                    <td>${service.name}</td>
                    <td><span class="badge badge-success">${service.status ? service.status.toUpperCase() : 'ACTIVE'}</span></td>
                    <td>${counterCount}</td>
                    <td>
                        <button class="btn btn-sm btn-info" onclick="editService(${service.id})">Edit</button>
        ...
        tbody.innerHTML = html || '<tr><td colspan="4" class="text-center">No services found</td></tr>';
    } catch (error) {
        console.error('Error loading services table:', error);
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-danger">Error loading services</td></tr>';
    }
}
...
async function loadStaffForAssignment() {
    try {
        const response = await api('../api/staff.php?action=get_all', 'GET');
...
// Open counter (set status to open)
async function openCounter(counterId) {
    try {
        const response = await api('../api/counters.php?action=set_status', 'POST', {
            counter_id: counterId,
            status: 'open'
        });

        if (response.success) {
            showAlert('Counter opened successfully', 'success');
            loadCounters();
        } else {
            showAlert(response.message || 'Error opening counter', 'danger');
        }
    } catch (error) {
        console.error('Error opening counter:', error);
        showAlert('Error opening counter', 'danger');
    }
}

// Close counter (set status to closed)
async function closeCounter(counterId) {
    try {
        const response = await api('../api/counters.php?action=set_status', 'POST', {
            counter_id: counterId,
            status: 'closed'
        ...
        if (response.success) {
            ...
            showAlert(response.message || 'Error closing counter', 'danger');
        }
    } catch (error) {
        console.error('Error closing counter:', error);
        showAlert('Error closing counter', 'danger');
    }
}

// Edit service
async function editService(serviceId) {
    if (!confirm('This will open the service editor. Continue?')) {
        return;
    }
    showAlert('Service editing feature coming soon', 'info');
}

// Delete service
async function deleteService(serviceId) {
    if (!confirm('Are you sure you want to delete this service? Counters using this service will be affected.')) {
        return;
    }

    try {
        const response = await api('../api/counters.php?action=delete_service', 'POST', {
        ...
        if (response.success) {
            ...
            showAlert(response.message || 'Error deleting service', 'danger');
        }
    } catch (error) {
        ...
        counterForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (counterId) {
                    ...
                    if (staffId) {
                        const staffResponse = await api('../api/counters.php?action=assign_staff', 'POST', {
                            counter_id: counterId,
                            staff_id: staffId
                        });
                        if (!staffResponse.success) {
                            showAlert('Counter updated but staff assignment failed', 'warning');
                        }
                    }
                } else {
                    // Add new counter
                    ...
                    if (response.success && staffId) {
                        ...
                        if (!staffResponse.success) {
                            showAlert('Counter created but staff assignment failed', 'warning');
                        }
                    }
                }

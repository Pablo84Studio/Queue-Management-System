...
class CounterManager {
    ...
    public function getCounterByStaffId($staff_id) {
        $query = "SELECT 
                    c.id,
                    c.name,
                    c.status,
    ...
    // Get counters by service
    public function getCountersByService($service_id) {
        $query = "SELECT 
                    c.id,
                    c.name,
                    c.status,
    ...
    public function deleteService($service_id) {
        try {
            // Check if service has counters
            $check_query = "SELECT COUNT(*) as count FROM counters WHERE service_id = ?";
            $stmt = $this->conn->prepare($check_query);
            if (!$stmt) {
                return ['success' => false, 'message' => 'Prepare failed: ' . $this->conn->error];
            }
            ...
            $result = $stmt->get_result()->fetch_assoc();
            if ($result['count'] > 0) {
                return ['success' => false, 'message' => 'Cannot delete service with assigned counters. Move or delete counters first.'];
            }
    ...
    public function assignAdminService($admin_id, $service_id) {
        try {
            ...
            $stmt->bind_param("ii", $admin_id, $service_id);
            $stmt->execute();
            
            if ($stmt->get_result()->num_rows > 0) {
                return ['success' => false, 'message' => 'Service already assigned to this admin'];
            }
            
            // Insert assignment
            $query = "INSERT INTO admin_services (admin_id, service_id) VALUES (?, ?)";
            $stmt = $this->conn->prepare($query);
            if (!$stmt) {
    ...
    public function removeAdminService($admin_id, $service_id) {
        try {
            $query = "DELETE FROM admin_services WHERE admin_id = ? AND service_id = ?";
            $stmt = $this->conn->prepare($query);
            if (!$stmt) {
                return ['success' => false, 'message' => 'Delete prepare failed: ' . $this->conn->error];
            }
            $stmt->bind_param("ii", $admin_id, $service_id);
            
            if (!$stmt->execute()) {
                return ['success' => false, 'message' => 'Delete execute failed: ' . $stmt->error];
            }
    ...
    public function getServicesByAdmin($admin_id) {
        $query = "SELECT s.id, s.name, s.description, s.status 
                  FROM services s
                  JOIN admin_services as_assign ON s.id = as_assign.service_id
                  WHERE as_assign.admin_id = ?
                  ORDER BY s.name ASC";
